/**
 * get_ugandan_implementations — Find Ugandan statutes implementing a specific EU directive/regulation
...
export interface GetUgandanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface UgandanImplementationResult {
  document_id: string;
...
export async function getUgandanImplementations(
  db: InstanceType<typeof Database>,
  input: GetUgandanImplementationsInput,
): Promise<ToolResponse<UgandanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as UgandanImplementationResult[];
