/**
 * search_eu_implementations — Search EU directives/regulations that have Ugandan implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_ugandan_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as ugandan_statute_count
  ...
  if (input.has_ugandan_implementation) {
    sql += ' HAVING ugandan_statute_count > 0';
  }
