#!/usr/bin/env tsx
/**
 * Database builder for Uganda Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Uganda Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Ugandan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'UG');
    insertMeta.run('source', 'ulii.org');
