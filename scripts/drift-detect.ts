#!/usr/bin/env tsx
/**
 * Drift detection for Uganda Law MCP.
 *
 * Checks if upstream Uganda Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Uganda Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'ugandan-law-mcp/1.0 drift-detect' },
      });
