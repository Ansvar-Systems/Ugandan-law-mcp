#!/usr/bin/env tsx
/**
 * Uganda Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from ulii.org (Akoma Ntoso HTML).
 *
 * Features:
 ...
 *
 * Data source: ulii.org (National Council for Law Reporting)
...
import { parseUgandaLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Uganda Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('====================================================\n');
  console.log(`  Source: ulii.org (National Council for Law Reporting)`);
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseUgandaLawHtml(html, act);
  ...
  console.log(`\n  Source:      ulii.org (Akoma Ntoso HTML)`);
